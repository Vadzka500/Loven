package com.sidspace.loven.home.domain.usecase

import com.sidspace.core.domain.model.DomainResult
import com.sidspace.core.domain.model.UserDomain
import com.sidspace.loven.home.domain.repository.HomeRepository
import javax.inject.Inject

class GetAccountUseCase @Inject constructor(private val homeRepository: HomeRepository) {
    suspend operator fun invoke(): DomainResult<UserDomain> = homeRepository.getAccount()
}
